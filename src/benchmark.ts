...
function benchmark(
  ...
  console.log(
    `  Setup Time (1 instance): ${(endSetup - startSetup).toFixed(4)}ms`,
  )
  ...
  console.log(
    `  Total Run Time (${iterations} iterations): ${totalTime.toFixed(4)}ms`,
  )
...
const scenario = (store: any) => {
  ...
  // 1. Select Species
  const event1 = {
    currentTarget: { name: 'hospSci', value: 'Citrus spp.' },
  } as any
...
try {
  const resultsOld = benchmark(
    'Baseline (Original deepSignal)',
    () => createBaselineStore(),
    scenario,
    1000,
  )
  ...
  const resultsNew = benchmark(
    'Restored Store (FastDeepSignal + shallow)',
    () => deepSignal(new RestoredStore()),
    scenario,
    1000,
  )
  ...
  console.log(
    `Performance: Restored Store is ${ratio.toFixed(2)}x FASTER than baseline`,
  )
} catch (e) {
